...
- lulucf_first_try, so tar_emi_exclLU = np.nansum([tar_emi_inclLU, -bl_onlyLU_taryr]) = np.nansum([648.3024610809886, - -261.57945]) = 909.8819110809886.
- tar_emi_exclLU = 909.8819110809886
...
If not both, inclLU and exclLU information are given calculate the 'other case' from the given case.
- tar_emi_exclLU is nan. So calculate it.
- lulucf_first_try, so tar_emi_exclLU = np.nansum([tar_emi_inclLU, -bl_onlyLU_taryr]) = np.nansum([549.0307132333927, - -261.57945]) = 810.6101632333928.
- tar_emi_exclLU = 810.6101632333928
