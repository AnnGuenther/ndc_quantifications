...
- tar_emi_inclLU
  ...
  - tar_emi_inclLU = intensity_growth * ndc_level_inclLU * (emi_cov_exclLU_refyr + bl_onlyLU_refyr) + emi_notcov_exclLU_taryr = 1.0 * 0.49 * (224.1487 + 117.94) + 0.0 = 167.62346300000002 MtCO2eq
- tar_emi_exclLU = ndc_value_exclLU = nan MtCO2eq
- tar_emi_inclLU = ndc_value_inclLU = 167.62346300000002 MtCO2eq
### Function calc_targets_inclLU_exclLU()
If not both, inclLU and exclLU information are given calculate the 'other case' from the given case.
- tar_emi_exclLU is nan. So calculate it.
- lulucf_first_try, so tar_emi_exclLU = np.nansum([tar_emi_inclLU, -bl_onlyLU_taryr]) = np.nansum([167.62346300000002, - 117.94]) = 49.68346300000002 MtCO2eq.
- tar_emi_exclLU = 49.68346300000002 MtCO2eq
- tar_emi_inclLU = 167.62346300000002 MtCO2eq