...
def import_table_to_class_metadata_country_year_matrix(path_to_table_csv):
    ...
    if path.isfile(str(path_to_table_csv)):
        ...
        for sheet_info in data_meta.index:
            ...
            if sheet_info in meta.nomenclature.oldname_to_attr:
                
                setattr(table, meta.nomenclature.oldname_to_attr[sheet_info], data_meta.loc[sheet_info, :].values[0])
            
            else:
                
                setattr(table, sheet_info, data_meta.loc[sheet_info, :].values[0])
        
        data_numeric = pd.read_csv(path_to_table_csv, skiprows=range(start_row_isos[0]), index_col=0)
