...
for ssp, file in \
    ...
    data = pd.read_csv(Path(meta.path.output, 'output_for_paper', 
        file, 'ndc_targets.csv'))
    tars_100[ssp] = data.loc[data.iso3 == iso3, :]
...
for ssp, add_x in zip(tars_not100.keys(), [-.6, -.3, 0, .3, .6]):
    ...
    colour_act = colours_ssps.loc[meta.ssps.scens.short_to_long[ssp.upper()], :].to_list()
    data = tars_100[ssp]
    ax1.plot([2033 + add_x, 2033 + add_x], 
        [float(data.loc[(data.tar_type_used == 'REI') & (data.condi == condi) & 
        (data.rge == xx), 'tar_emi_exclLU']) for xx in ['best', 'worst']], 
        color=colour_act, label='__nolegend__',
...
for axa in [ax1, ax2]:
    ...
    year, txt = 2005, 'base year'
    axa.text(year, ypos, txt, ha='center', va='bottom', fontweight='bold')
    axa.plot([2030, 2030], YL, 'k:', linewidth=.3)
    
    if axa == ax1:
        axa.plot([2032, 2032], YL, 'k:', linewidth=.3)
        
        axa.text(2031, np.diff(YL) * .02, 'estimated coverage',
            rotation=90, ha='center', va='bottom')
        axa.text(2033, np.diff(YL) * .02, '100% coverage',
            rotation=90, ha='center', va='bottom')
