...
Only covered emissions (at least for the non-NDC data).
"""
...
for ssp in meta.ssps.scens.long:
    table = hpf.import_table_to_class_metadata_country_year_matrix(
        Path(meta.path.pc_cov, f"KYOTOGHG_IPCM0EL_COV_EMI_{ssp}FILLED_CORR.csv"))
    ...
    emi_ssps_tot['exclLU'][ssp] = table.data
    emi_ssps_tot['inclLU'][ssp] = hpf.copy_table(table).data.add(lulucf, fill_value=0)
...
sums_per_year_tot = pd.DataFrame(index=years)
for year in years:
        ...
        if data_ndcs != 0:
            ...
            if len(isos_with_data) > 0:
                ...
                for ssp in meta.ssps.scens.long:
                    ...
                    # Global share (SSP2)
                    sums_per_year_tot.loc[year, f"{case}_{ssp}"] = emi_ssps_tot[case][ssp].loc[isos_with_data, year].sum()
...
for year in number_of_values.index:
    for case, df in ['exclLU', df_exclLU], ['inclLU', df_inclLU]:
        ...
        emi_ssp2 = emi_ssps_tot[case][ssp].loc[:, year]
        diff_ssp2 = data_act[f"{case}_ndc"] - data_act[f"{case}_{ssp2}"]
        share_ssp2 = sums_per_year_tot.loc[year, f"{case}_{ssp2}"]/emi_ssp2.reindex(index=meta.isos.EARTH).sum(axis=0)*100
...
hpf.write_text_to_file(txt, 
   Path(meta.path.main, 'data', 'other', 'ndc_emissions_vs_ssps_externalDataOnlyCoveredEmi.txt'))
