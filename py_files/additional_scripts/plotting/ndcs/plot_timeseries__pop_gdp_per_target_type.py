...
def plotting_both_ssps_as_markers():
    ...
    for count, tpe in zip(range(len(tpes)), tpes):
        ...
        for ssp in data.keys():
            ...
            vals_calc = 1e-3 * data[ssp].data.loc[isos_calc, years_int].sum(axis=0)
            vals_nan_calc = 1e-3 * data[ssp].data.loc[ctrs_none_calc, years_int].sum(axis=0)
            vals_all_calc = 1e-3 * data[ssp].data.loc[meta.isos.EARTH, years_int].sum(axis=0)
    ...
    for count, tpe in zip(range(len(tpes)), tpes):
        ...
        for ssp in data.keys():
            ...
            vals_orig = 1e-3 * data[ssp].data.loc[isos_orig, years_int].sum(axis=0)
            vals_nan_orig = 1e-3 * data[ssp].data.loc[ctrs_none_orig, years_int].sum(axis=0)
            vals_all_orig = 1e-3 * data[ssp].data.loc[meta.isos.EARTH, years_int].sum(axis=0)
    ...
    share_REI_RBU = 100. * 1e-3 * data[next(xx for xx in data.keys() if 'SSP2' in xx)].data. \
        loc[isos_per_type_orig['REI_RBU'], years_rei].sum(axis=0).div(emi_REI)
    YL = ax1.get_ylim()
    ax1.set_ylim()
...
for what, what_info, what_label in \
    ...
    path_to_file = Path(path_to_folder, f"time_series_{what}_per_target_type_orig_and_calc_range_of_ssps.png")
