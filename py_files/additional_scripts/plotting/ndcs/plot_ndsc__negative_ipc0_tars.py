...
for iso3 in meta.isos.EARTH:
    ...
    tars_iso3 = tars_calc.loc[tars_calc.iso3 == iso3, :]
    if len(tars_iso3) > 0:
        tar_type_calc = tars_iso3.tar_type_calc.unique()[0]
        ...
        if any(tars_iso3.tar_emi_inclLU.astype(float) < 0.):
            ...
            ax1.scatter(
                tars_iso3.loc[tars_iso3.tar_type_used == tar_type_calc, 'taryr'].astype(float).values,
                tars_iso3.loc[tars_iso3.tar_type_used == tar_type_calc, 'tar_emi_inclLU'].astype(float).values, marker='s', color='b', label='tar_emi_inclLU')
            ax1.scatter(
                tars_iso3.loc[tars_iso3.tar_type_used == tar_type_calc, 'taryr'].astype(float).values,
                tars_iso3.loc[tars_iso3.tar_type_used == tar_type_calc, 'tar_emi_exclLU'].astype(float).values, marker='*', color='b', label='tar_emi_exclLU')
            ax1.scatter(
                tars_iso3.loc[tars_iso3.tar_type_used == tar_type_calc, 'refyr'].astype(float).values,
                tars_iso3.loc[tars_iso3.tar_type_used == tar_type_calc, 'emi_bl_onlyLU_refyr'].astype(float).values, marker='^', color='g', label='LU used refyr')
            ax1.scatter(
                tars_iso3.loc[tars_iso3.tar_type_used == tar_type_calc, 'taryr'].astype(float).values,
                tars_iso3.loc[tars_iso3.tar_type_used == tar_type_calc, 'emi_bl_onlyLU_taryr'].astype(float).values, marker='o', color='g', label='LU used taryr')
