...
# %%
for iso3 in ['USA']: # meta.isos.EARTH:
...
#                bottom_emi_pop = vals_combi_pop.loc[combi, :].add(bottom_emi_pop).values[0]
        ...
        if len(lu_srces) > 0:
            ...
            txt += \
                ...
                "\n \\newline \\textbf{LULUCF: can be sink or source, uncertainties, difference between data by different sources!}" + \
        ...
        if has_ndc not in ['NDC', 'INDC']:
            ...
            def plot_ndcs():
                    ...
                    for what in ['cov100', 'covest']:
                        ...
                        for ssp_short, ssp_long in zip(meta.ssps.scens.short, meta.ssps.scens.long):
                            ...
                            ax_ts.plot(
                                ...
                                linewidth=linewdth, color=color_ssp,
                ...
                for yloc, txt in zip(ylocs[1:],
                    [f'thinner lines per dmSSP: prio NDC (type_reclass = {type_reclass})',
                     ...
                     'shaded area for conditionality range (default; prio NDC vs. SSPs)']):
                    ax_ts.text(XL[0], YL[1] + yloc*np.diff(YL), txt, ha='left', va='bottom', fontsize=8)
                XL = ax_vert.get_xlim()
                YL = ax_vert.get_ylim()
                yloc = YL[1] - .02*np.diff(YL)
                for xx in np.arange(-.7, 100, 20):
                    ax_vert.plot([xx, xx + 9.5], [yloc, yloc], color=colour_prioNDCs, linewidth=2)
                for xx in np.arange(9.3, 100, 20):
                    ax_vert.plot([xx, xx + 9.5], [yloc, yloc], color=colour_prioSSPs, linewidth=2)
                ...
                ax_ts.set_xlabel('year', fontweight='bold')
                uni_lbl = f"{units_iso['emi']['label']} AR4"
            ...
            if type_main != type_reclass:
                txt += \
                    ...
                    f"\n We reclassify a country's target when a country has, e.g., an RBY~/ RBU target ({tar_to_long['RBY']}~/ {tar_to_long['RBU']}), " + \
                    "and the base year~/ BAU emissions are provided." + \
                    "\n In this case it can be quantified based on the given emissions, and is reclassified from type\_main RBY~/ RBU to " + \
                    ...
                    "\n The baseline emissions or target values provided in an NDC are used to quantify a country's target if possible." + \
                    "\n However, for comparison purposes and as uncertainty range, if type\_main is not ABS, " + \
                    "we also quantify the target based on 'external' emissions (PRIMAP-hist, dmSSPs)."
            ...
            txt += \
                "\n \\newline \\textbf{Put in the covered sectors / gases in the text as well! And some information like: " + \
            ...
            txt += \
                (f"\n\n As {ctr_the_lc} is part of the NDC by the European Union (28), " + \
        ...
        primap_link_paper_text = "G{\"u}tschow et al. (2016)"
        txt += \
        ...
        primap_crf_data_text = "G{\"u}tschow et al., 2020"
