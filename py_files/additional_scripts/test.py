...
def plotting_together_types_onePlot_diff():
    ...
    tars_uncondi_worst = pd.DataFrame(columns=ssp1_5, dtype='float64')
    tars_uncondi_best = pd.DataFrame(columns=ssp1_5, dtype='float64')
    tars_condi_worst = pd.DataFrame(columns=ssp1_5, dtype='float64')
    tars_condi_best = pd.DataFrame(columns=ssp1_5, dtype='float64')
    ...
    for folder, folder100, ssps in zip(folders_calc, folders_calc100, meta.ssps.scens.short):
        ...
        data = ptws_calc[ssps]
        what = 'calc'
        tars_uncondi_worst.loc[what, ssps] = data.loc[(data.group == 'EARTH') & (data.condi == 'unconditional') & (data.rge == 'worst'), '2030'].max()
        tars_uncondi_best.loc[what, ssps] = data.loc[(data.group == 'EARTH') & (data.condi == 'unconditional') & (data.rge == 'best'), '2030'].max()
        tars_condi_worst.loc[what, ssps] = data.loc[(data.group == 'EARTH') & (data.condi == 'conditional') & (data.rge == 'worst'), '2030'].max()
        tars_condi_best.loc[what, ssps] = data.loc[(data.group == 'EARTH') & (data.condi == 'conditional') & (data.rge == 'best'), '2030'].max()
    ...
    diff_med = (tars_uncondi_worst-tars_condi_best)*1e-3
    [print(f"% Difference between uncondi worst & condi best: min/max for the different options, for {xx}: " +
           f"{diff_med.loc[:, xx].min() :.1f} to {diff_med.loc[:, xx].max() :.1f} Gt CO2eq AR4") for xx in tars_uncondi_worst.columns]
    
    print("\nDifference between the default 100% runs with LULUCF prio CRF, and with FAO:")
    for ssp in tars_uncondi_worst.columns:
        print(f"type_calc ({ssp}): " +
            f"unconditional worst {1e-3*(tars_uncondi_worst.loc['calc_100pc', ssp] - tars_uncondi_worst.loc['calc_fao', ssp]) :.1f}; " +
            f"conditional best {1e-3*(tars_condi_best.loc['calc_100pc', ssp] - tars_condi_best.loc['calc_fao', ssp]) :.1f} Gt CO2eq")
        print(f"type_orig ({ssp}): " +
            f"unconditional worst {1e-3*(tars_uncondi_worst.loc['orig_100pc', ssp] - tars_uncondi_worst.loc['orig_fao', ssp]) :.1f}; " +
            f"conditional best {1e-3*(tars_condi_best.loc['orig_100pc', ssp] - tars_condi_best.loc['orig_fao', ssp]) :.1f} Gt CO2eq")
    
    rge_SSP2 = np.percentile(1e-3*(tars_uncondi_worst.loc[:, 'SSP2'] - tars_condi_best.loc[:, 'SSP2']), [10, 50, 90])
    rge_SSP1to4 = np.percentile(1e-3*(tars_uncondi_worst.loc[:, ssp1_4] - tars_condi_best.loc[:, ssp1_4]), [10, 50, 90])
    print(f"\n% Range for SSP2 (10, 50, 90%): {rge_SSP2} Gt CO2eq")
    print(f"% Range for SSP1-4 (10, 50, 90%): {rge_SSP1to4} Gt CO2eq")
    
    print(f"\n% Range of unconditional worst (SSP1-4, 10, 50, 90%): {np.percentile(tars_uncondi_worst.loc[:, ssp1_4]/1000, [10, 50, 90])} Gt CO2 eq")
    print(f"% Range of unconditional best (SSP1-4, 10, 50, 90%): {np.percentile(tars_uncondi_best.loc[:, ssp1_4]/1000, [10, 50, 90])} Gt CO2 eq")
    print(f"% Range of conditional worst (SSP1-4, 10, 50, 90%): {np.percentile(tars_condi_worst.loc[:, ssp1_4]/1000, [10, 50, 90])} Gt CO2 eq")
    print(f"% Range of conditional best (SSP1-4, 10, 50, 90%): {np.percentile(tars_condi_best.loc[:, ssp1_4]/1000, [10, 50, 90])} Gt CO2 eq")
    
    tars_SSP1to4 = np.percentile((tars_uncondi_worst.loc[:, ssp1_4].values,
                                  tars_condi_best.loc[:, ssp1_4].values), [10, 50, 90])/1000
    print(f"\n% Range of SSP1-4 quantifications (uncondi worst and condi best): {tars_SSP1to4} Gt CO2 eq")
    
    rge_condi_SSP1to4 = np.percentile(tars_uncondi_worst.loc[:, ssp1_4].add(
        -tars_condi_best.loc[:, ssp1_4]).values, [10, 50, 90])/1000
    print(f"\n% Range between unconditional worst and conditional best targets (SSP1-4, 10, 50, 90%): {rge_condi_SSP1to4} Gt CO2eq")
   
    tars_uncondi_worst.drop(columns=['SSP5'], inplace=True)
    tars_condi_best.drop(columns=['SSP5'], inplace=True)
    tars_uncondi_worst.loc[:, 'median'] = tars_uncondi_worst.median(axis=1)
    tars_condi_best.loc[:, 'median'] = tars_condi_best.median(axis=1)
